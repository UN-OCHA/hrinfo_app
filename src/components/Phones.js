...
class Phones extends React.Component {
  ...
  handleChange(number, type, v) {
    ...
    else if (type === 'dialling_code') {
      let dialling_codes = this.state.dialling_codes;
      dialling_codes[number] = v;
      this.setState({
        dialling_codes: dialling_codes
      });
    }
    else if (type === 'number') {
      let numbers = this.state.numbers;
      numbers[number] = v.target.value;
      this.setState({
        numbers: numbers
      });
    }
  ...
  removeFileRow (number) {
    this.setState((prevState, props) => {
      prevState.types.splice(number, 1);
      prevState.dialling_codes.splice(number, 1);
      prevState.numbers.splice(number, 1);
      return {
        types: prevState.types,
        dialling_codes: prevState.dialling_codes,
