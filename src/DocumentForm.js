...
class DocumentForm extends React.Component {
    ...
    handleSelectChange(name, selected) {
        let doc = this.state.doc;
        doc[name] = selected;
        let hasOperation = this.state.doc.hasOperation
            ? this.state.doc.hasOperation
            : false;
        if (name === 'spaces') {
    ...
    validateForm() {
        ...
        if (this.isValid(doc.language) && this.isValid(doc.spaces) && this.isValid(doc.label) && ((this.state.type === 'documents' && this.isValid(doc.document_type)) || (this.state.type === 'infographics' && this.isValid(doc.infographic_type))) && this.isValid(doc.publication_date) && this.isValid(doc.files) && this.isValid(doc.organizations)) {
            return true;
        } else {
            return false;
        }
    ...
    handleSubmit(event, isDraft = 0) {
        ...
        if (!isValid) {
            this.setState({status: 'was-validated'});
            return;
        }
    ...
    onEditorStateChange(editorState) {
        ...
        let doc = this.state.doc;
        doc.body = html;
        this.setState({editorState, doc: doc});
    }
